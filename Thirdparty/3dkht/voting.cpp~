                             ...
                             const double theta, const double phi)
{
   ...
   for (rho_index = rho_start_index, rho = 0.0 ;; rho_index += inc_rho_index, rho += inc_rho)
   {
      ...
      if ((votes = (ACCUM_BIN_TYPE)(gauss)) >= (ACCUM_BIN_TYPE)(0.0)) {
...
void kernel_calculation(octree_t &node, accumulatorball_t &accum, std::vector<kernel_t> &used_kernels, double max_point_distance)
{
   kernel_t kernel;
   kernel.node = &node;
   
   if ((node.m_centroid.Normalized() & node.normal1) < 0)  node.normal1 *= -1.0;

   kernel.phi = acos(node.normal1.z);
   kernel.theta = atan2(node.normal1.y, node.normal1.x);
...
void kernels(octree_t &root, accumulatorball_t &accumulator , std::vector<kernel_t> &used_kernels, double max_point_distance)
{
   if (root.coplanar)
   {
...
void voting(octree_t &root, accumulatorball_t &accumulator, std::vector<bin_t> &used_bins, double max_point_distance)
{
   ...
   for (unsigned short i = 0; i < used_kernels.size(); i++)
